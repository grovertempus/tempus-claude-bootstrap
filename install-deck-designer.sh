...
if [ ! -t 0 ]; then
  ...
  echo ""
  echo "  curl -fsSL -o /tmp/tempus-setup.sh https://raw.githubusercontent.com/grovertempus/tempus-claude-bootstrap/main/install-deck-designer.sh && bash /tmp/tempus-setup.sh"
  echo ""
  exit 1
fi
...
print_success() {
  ...
  echo "============================================"
  echo ""
  echo "Everything is installed. Just one last step"
  echo "- sign in:"
  echo ""
  echo "  1. Open VS Code"
  ...
  echo ""
  echo "Once signed in, type /deck-designer to start"
  echo "building a presentation."
  echo ""
